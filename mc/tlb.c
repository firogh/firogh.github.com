...
void print_tlbcam(void)
{
	...
	for (i = 0; i < num_cam; i++) {
		...
		// Rupam printf("entry %02d: V: %d EPN 0x%08x RPN 0x%08llx size:",
			//i, (valid == 0) ? 0 : 1, (unsigned int)epn,
			//(unsigned long long)rpn);
		printf("entry %02d: V: %d IP: %d EPN 0x%08x RPN 0x%08llx size:",
			i, (valid == 0) ? 0 : 1, (iprot == 0) ? 0 : 1, (unsigned int)epn,
			(unsigned long long)rpn);
	     ...
	     u8 ts, u8 esel, u8 tsize, u8 iprot)
{
    ...
    debug("### Rupam %s: tlb %d epn %#x, rpn %#lx, perms %#x, wimge %#x, ts %d, esel %d, tsize %d, iprot %d\n", 
            __func__, tlb, epn, rpn, perms, wimge, ts, esel, tsize, iprot);
	if (tlb == 1)
		use_tlb_cam(esel);
